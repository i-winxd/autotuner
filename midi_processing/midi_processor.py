...
def representation_to_midi_file(midi_representation: MidiRepresentation) -> mido.MidiFile:
    ...
    # TODO: this is a bug fix. whatever this is, you should probably push it
    previous_tc = 0
...
def midi_to_representation(midi_file: mido.MidiFile) -> MidiRepresentation:
    ...
    logging.debug(f"Track names: {track_names}")
    for i, track in enumerate(midi_file.tracks):
        logging.debug(f"Track {i} has length {track.__len__()}")
        notes: list[Note] = []
        accumulated_time = 0
        # [PITCH, CHANNEL]
    ...
    for i, track in enumerate(midi.tracks):
        for msg in track:
            ...
            if msg.type == 'set_tempo':
                ...
                logging.debug(f"Tempo change detected in track {i}: {bpm} @ {beats}")
        ...
        for msg in track:
            ...
            if msg.type == 'time_signature':
                ...
                time_signature_changes.append(TimeSignature(numerator=msg.numerator, denominator=msg.denominator,
                                                            beat=beats))
                logging.debug(f"Time signature change in track {i}: {msg.numerator}/{msg.denominator}")
